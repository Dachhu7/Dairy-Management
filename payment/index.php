...
<form class=" form-inline" method="post" action="">
    <div class="control-group">
        ...
        <div id="datetimepicker1" class="controls input-append date" style="margin-left: 20px">
            <input class="input-xlarge" type="text" data-format="yyyy-mm-dd hh:mm:ss"  placeholder="yyyy-mm-dd" name='from' value='<?php echo stripslashes($start); ?>'/> 
            <span class="add-on">
        ...
        <div id="datetimepicker2" class="controls input-append date" style="margin-left: 20px">
            <input class="input-xlarge" type="text" data-format="yyyy-mm-dd hh:mm:ss"  placeholder="yyyy-mm-dd" name='to' value='<?php echo $end; ?>'/> 
            <span class="add-on">
                <i data-time-icon="icon-time" data-date-icon="icon-calendar">
                </i>
            </span>
...
<table class="table table-hover table-striped table-condensed table-bordered">
    ...
    <tbody>
        ...
        if (isset($_POST['from'])) {
            ...
            $farmers = mysqli_query($conn,"select f_no,f_name,f_ac,last_paid from farmers") or die("unable to fetch records" . mysqli_error($conn));
            $i = 0;
            $total = 0;
            while ($farmer = mysqli_fetch_array($farmers)) {
                //$i+=1;
                $f_no = $farmer['f_no'];
                $result = mysqli_query($conn,"SELECT Amount FROM `delivery` WHERE r_f_no=$f_no and `r_dt` >='$start' and `r_dt` <= '$end'") or trigger_error(mysqli_error($conn));
                ...
                echo "<td valign='top'>" . nl2br($farmer['f_ac']) . "</td>";
            ...
            echo "<tr class='success'><td><strong>Total</strong></td><td><strong>All</strong><td>--</td><td>--</td><td>--</td><td>$total</td><td></td></tr>";
        }
        ?>
    </tbody>
</table>
